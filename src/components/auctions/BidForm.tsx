...
const BidForm: React.FC<BidFormProps> = ({
  ...
  const handleBidSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if bid is within 2 minutes of auction end time
      const now = new Date();
      const end = new Date(endDate);
      const timeDiffInMinutes = (end.getTime() - now.getTime()) / (1000 * 60);

      if (timeDiffInMinutes <= 2 && timeDiffInMinutes > 0) {
        try {
          await axios.get(`http://localhost:8080/auction/extendAuctionTime/${auctionId}`, {
            withCredentials: true,
          });
          console.log("Auction time extended due to bid placed near end.");
        } catch (extendErr) {
          console.error("Failed to extend auction time:", extendErr);
        }
      }
